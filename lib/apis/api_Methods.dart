...
import 'api_models/get_relation_requests_model.dart';
...
class ApiMethods {
  ...
  static Future<UserDataModel?> addNewMember(
      {File? image, String? imageKey, Map<String, dynamic>? bodyParams}) async {
    http.Response? response = await MyHttp.postMethod(
        url: ApiUrlConstants.endPointOfAddNewMember,
        /*image: image,
        imageKey: imageKey,*/
        bodyParams: bodyParams);
    if (response != null) {
  ...
  static Future<UserDataModel?> confirmRelation(
      {File? image, String? imageKey, Map<String, dynamic>? bodyParams}) async {
    http.Response? response = await MyHttp.postMethod(
        url: ApiUrlConstants.endPointOfConfirmRelation, bodyParams: bodyParams);
    if (response != null) {
      return UserDataModel.fromJson(jsonDecode(response.body));
    }
    return null;
  ...
  static Future<UserDataModel?> addPost({
  ...
  static Future<GetRelationRequestsModel?> getRelationRequests(
      {File? image, String? imageKey, Map<String, dynamic>? bodyParams}) async {
    http.Response? response = await MyHttp.getMethod(
        url: ApiUrlConstants.endPointOfGetRelationRequests);
    if (response != null) {
      return GetRelationRequestsModel.fromJson(jsonDecode(response.body));
    }
    return null;
  }
}
